...
# Resize, Normalisierung und Augmentation auf Trainingsdaten anwenden
train_dataset = train_dataset.map(lambda x, y: (tf.image.resize_with_pad(x, 224, 224), y))
train_dataset = train_dataset.map(lambda x, y: (normalize(x), y))
train_dataset = train_dataset.map(lambda x, y: (data_augmentation(x, training=True), y))

# Resize und Normalisierung auf Validierungsdaten anwenden
val_dataset = val_dataset.map(lambda x, y: (tf.image.resize_with_pad(x, 224, 224), y))
...
model.save("model2_normalized.keras")